...
def get_directors(tag):
  ...
  directors = list(map(lambda d: d.a["title"], tag.find_all(class_="mc-director")[0].find_all(class_="nb")))
...
def get_data(user_id, lang):
  ...
  eof = False
  n = 1
  FA = "https://www.filmaffinity.com/" + lang + "/userratings.php?user_id={id}&p={n}&orderby=4"
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Generates csv compatible with LetterBoxd from Filmaffinity user's id.")
  parser.add_argument("id", help="User's id")
  parser.add_argument("--csv", nargs=1, help="Name of export FILE", metavar="FILE")
  parser.add_argument("--lang", nargs=1, help="Language for exporting", metavar="LANG", default=["en"], choices={"es", "en"})

  args  = parser.parse_args()
  export_file  = args.csv[0] if args.csv else "filmAffinity_{lang}_{id}.csv".format(id = args.id, lang = args.lang[0])

  try:
    set_locale(args.lang[0])
  except locale.Error:
    print("Could not set locale for \'{lang}\' and UTF-8 encoding.".format(lang = args.lang[0]))
